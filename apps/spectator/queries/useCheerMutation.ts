...
//   return useMutation({
//     mutationFn: ({ cheerCount, gameId, gameTeamId }: Options) =>
//       postCheer({ gameId, gameTeamId, cheerCount }),
//     onMutate: async ({ gameId, gameTeamId, cheerCount }) => {
//       await queryClient.cancelQueries({ queryKey: ['game-cheer', gameId] });
...
//       queryClient.setQueryData(
//         ['game-cheer', gameId],
//         (oldData: GameCheerType[]) =>
//           oldData.map(item =>
//             item.gameTeamId === gameTeamId
//               ? { ...item, cheerCount: item.cheerCount + cheerCount }
//               : item,
//           ),
//       );
