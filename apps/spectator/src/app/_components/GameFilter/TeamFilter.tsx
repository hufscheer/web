...
export default function TeamFilter({ leagueId, round }: LeagueTeamFilterProps) {
  ...
  const handleRouter = (event: MouseEvent<HTMLButtonElement>, selectedTeamId: number) => {
    ...
    <div className={styles.teamWrapper}>
      <button className={clsx(styles.expandable.button)} onClick={toggleExpand}>
        <Icon
          ...
          className={clsx(styles.expandable.caret, isExpanded && styles.expandable.caretFocused)}
        />
      </button>

      <ul
        ref={scrollRef}
        className={clsx(styles.leagueTeam.list, isExpanded && styles.leagueTeam.listExpanded)}
      >
        {data.map((team) => (
