  ...
  return (
    <>
      {cheerTalkList.length === 0 && socketTalkList.length === 0 && (
        <div className={styles.emptyMsg}>지금 우리 팀에게 첫 응원톡을 남겨주세요! 💪</div>
      )}
      <div className={styles.list.container}>
        <ul ref={scrollRef} className={styles.list.content}>
          ...
          {cheerTalkList.map((talk) => (
            <CheerTalkItemMemo key={`cheer-${talk.cheerTalkId}`} hasMenu {...talk} />
          ))}

          {/* Socket */}
          {socketTalkList.map((talk) => (
            <CheerTalkItemMemo key={`socket-${talk.cheerTalkId}`} hasMenu {...talk} />
          ))}
          <li ref={bottomRef} />
        </ul>
