...
export const getGameTimelineById = async (gameId: string) => {
  const { data } = await instance.get<GameTimelineType[]>(`/games/${gameId}/timeline`);

  return data;
};

export const getGameLineupById = async (gameId: string) => {
  const { data } = await instance.get<GameLineupType[]>(`/games/${gameId}/lineup`);

  return data;
};

export const getGameCheerTalkById = async (gameId: string, cursor: number | string, size = 20) => {
