import { GameTeamType, useCreateReplacementTimeline, useGame, useGameLineup } from '@hcc/api';
import {
  Button,
  Form,
  FormControl,
...
const ReplacementForm = ({ gameId, onClose, quarter }: ReplacementFormProps) => {
  ...
  const { mutate: createReplacementTimelineMutation, isPending } = useCreateReplacementTimeline();
  ...
  const players = useMemo(() => {
    return (
      lineupPlayers?.find((lineup) => lineup.gameTeamId === Number(gameTeamId))?.candidatePlayers ??
      []
    );
  ...
  const playingPlayers = useMemo(() => {
    return (
      lineupPlayers?.find((lineup) => lineup.gameTeamId === Number(gameTeamId))?.starterPlayers ??
      []
    );
  }, [lineupPlayers, gameTeamId]);

  if (!quarter)
    return <p className={styles.emptyQuarterMessage}>상태 변경을 통해 쿼터 상태를 지정해주세요.</p>;
      ...
      <form className={styles.form} onSubmit={methods.handleSubmit(onSubmit)}>
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <FormControl>
                    <SelectTrigger type="button">
                      <SelectValue>{QUARTERS_DB[field.value as QUARTER_KEY]}</SelectValue>
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
              ...
              <FormItem>
                ...
                >
                  ...
                  <FormControl>
                    <SelectTrigger type="button">
                      <SelectValue>
                        {teams.find((team) => team.gameTeamId === Number(field.value))
                          ?.gameTeamName ?? '팀 선택'}
                      </SelectValue>
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
                    {teams.map((team) => (
                      <SelectItem key={team.gameTeamId} value={team.gameTeamId.toString()}>
                        {team.gameTeamName}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        <section className={styles.section}>
          <FormField
            ...
            render={({ field }) => {
              const selectedPlayer = players.find((player) => player.id === Number(field.value));
        ...
        <Button disabled={isPending} type="submit" fontWeight="semibold" fullWidth>
