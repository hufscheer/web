...
const RecordDeleteMenu = ({ gameId, record }: RecordDeleteMenuProps) => {
      ...
      <span className={styles.recordContainer}>
        <div className={styles.recordTeamContainer}>
          <div className={styles.recordTeamLogo}>
            <Image src={record.teamImageUrl} alt={record.teamName} fill draggable={false} />
          </div>
  ...
  const TextRecordItem = () => {
    return (
      <span className={styles.recordContainer}>
        <p className={styles.recordText}>{record.progressRecord?.gameProgressType}</p>
      </span>
    );
  };
  ...
  return (
    <AlertDialog
      title="이 타임라인을 삭제할게요"
      description={record.type === 'GAME_PROGRESS' ? <TextRecordItem /> : <EventRecordItem />}
