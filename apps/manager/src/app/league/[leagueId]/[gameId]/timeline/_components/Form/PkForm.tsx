import { GameTeamType, useGame, useGameLineupPlaying, useCreatePkTimeline } from '@hcc/api';
import {
  Button,
  Form,
  FormControl,
...
const PkForm = ({ gameId, onClose }: PkFormProps) => {
  ...
  const players = useMemo(() => {
    return (
      lineupPlayingPlayers?.find((lineup) => lineup.gameTeamId.toString() === gameTeamId)
        ?.gameTeamPlayers ?? []
    );
  ...
  return (
    <Form {...methods}>
      <form className={styles.form} onSubmit={methods.handleSubmit(onSubmit)}>
                ...
                >
                  ...
                  <FormControl>
                    <SelectTrigger type="button">
                      <SelectValue>
                        {teams.find((team) => team.gameTeamId.toString() === field.value)
                          ?.gameTeamName ?? '팀 선택'}
                      </SelectValue>
                    </SelectTrigger>
                  </FormControl>
              ...
              <FormItem>
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <FormControl>
                    <SelectTrigger type="button">
                      <SelectValue>
                        {players.find((player) => player.id.toString() === field.value)
                          ?.playerName ?? '선수 선택'}
                      </SelectValue>
                    </SelectTrigger>
                  </FormControl>
            ...
            render={({ field }) => (
              <FormItem>
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <FormControl>
                    <SelectTrigger type="button">
                      <SelectValue>{field.value === '1' ? '성공' : '실패'}</SelectValue>
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
        ...
        <Button type="submit" fontWeight="semibold" fullWidth disabled={isPending}>
