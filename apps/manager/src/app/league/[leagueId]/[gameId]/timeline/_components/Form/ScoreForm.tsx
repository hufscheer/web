import { GameTeamType, useGame, useGameLineupPlaying, useCreateScoreTimeline } from '@hcc/api';
import {
  Button,
  Form,
  FormControl,
...
import { scoreDefaultValues, scoreFormSchema, ScoreFormSchema } from '../../_types';
...
const ScoreForm = ({ gameId, onClose, quarter }: ScoreFormProps) => {
  ...
  const { mutate: createScoreTimelineMutation, isPending } = useCreateScoreTimeline();
  ...
  const players = useMemo(() => {
    return (
      lineupPlayingPlayers?.find((lineup) => lineup.gameTeamId === Number(gameTeamId))
        ?.gameTeamPlayers ?? []
    );
