'use client';
import { useLeagueCheerTalk, useLeagueCheerTalkReported, useUpdateCheerTalkBlock } from '@hcc/api';
...
const Component = ({ leagueId }: ComponentProps) => {
  ...
  const AllCheerTalkContent = () => {
    const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useLeagueCheerTalk(leagueId);
    return (
      <CheerTalkList
  ...
  return (
    <Layout
      ...
      navigationMenu={<Link href={`/league/${leagueId}/cheer-talk/blocked`}>가려진 목록</Link>}
    >
      <Tabs className={styles.tab} defaultValue="ALL">
        ...
        <Suspense fallback={null}>
          {tabs.map((tab) => (
            <Tabs.Content key={tab.key} value={tab.key} className={styles.tabContent}>
              {tab.renderer()}
            </Tabs.Content>
          ))}
        </Suspense>
