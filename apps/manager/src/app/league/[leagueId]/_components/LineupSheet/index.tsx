...
import { ArrowCircleDownIcon, ArrowCircleUpIcon, CaptainIcon } from '@hcc/icons';
import {
  BottomSheet,
  BottomSheetContent,
...
const LineupSheet = ({ gameId, teamId }: LineupUpdateSheetProps) => {
  ...
  const teamName: string =
    game?.gameTeams.find((gameTeam) => gameTeam.gameTeamId === Number(teamId))?.gameTeamName || '';

  const currentPlayingLineup: GameTeamPlayerType[] =
    lineupList?.find((lineup) => lineup.gameTeamId === Number(teamId))?.starterPlayers || [];

  const candidateLineup: GameTeamPlayerType[] =
    lineupList?.find((lineup) => lineup.gameTeamId === Number(teamId))?.candidatePlayers || [];

  const { mutate: updateLineupCaptainRegister, isPending: isCaptainRegisterPending } =
    useUpdateLineupCaptainRegister();
  const { mutate: updateLineupCaptainRevoke, isPending: isCaptainRevokePending } =
    useUpdateLineupCaptainRevoke();
  const handleChangeCaptain = (playerId: number, isCaptain: boolean) => {
    if (isCaptain) {
      ...
      updateLineupCaptainRevoke(
        ...
        {
          onSuccess: () => toast({ title: '라인업 수정 완료', variant: 'destructive' }),
        },
      );
    } else {
  ...
  const handleAllCandidatesToStarting = () => {
    Promise.all(
      candidateLineup.map((player) => updateLineupStarter({ gameId, lineupPlayerId: player.id })),
    )
  ...
  return (
    <BottomSheet open={open} onOpenChange={setOpen}>
      ...
      <BottomSheetContent className={styles.content}>
        <BottomSheetHeader className={styles.header} style={{ display: 'flex' }}>
          <div className={styles.headerInfo}>
            <BottomSheetTitle className={styles.title}>라인업 수정</BottomSheetTitle>
        ...
        <section className={styles.section}>
            ...
            {currentPlayingLineup.map((player) => (
              <li className={styles.playerItem} key={player.id}>
                <div className={styles.card}>
                  <button onClick={() => handleChangeCaptain(player.id, player.isCaptain)}>
                    <Icon
                      role="button"
                      source={CaptainIcon}
                      width={24}
          ...
          <div className={styles.division}>
            <h3>후보</h3>
            <button className={styles.selectAllButton} onClick={handleAllCandidatesToStarting}>
              모두 선발로 올리기
            </button>
          </div>
          <ul className={styles.playerList}>
