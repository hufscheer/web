...
export const GameForm = ({
  ...
  const getTeamName = (teamId: string) => {
    return type === 'CREATE'
      ...
      : game?.gameTeams.find((t) => t.gameTeamId.toString() === teamId)?.gameTeamName;
  };
              ...
              <FormItem>
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <FormControl>
                    <SelectTrigger>
                      <span>
                        {field.value && (field.value === '2' ? '결승' : `${field.value}강`)}
                      </span>
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
                    {ROUND_OPTIONS.filter((item) => league.maxRound >= item.round).map((item) => (
                  ...
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button colorScheme="outline" justify="between" fullWidth>
                        <span>
                          {field.value ? formatTime(field.value, 'YYYY년 MM월 DD일') : ''}
                        </span>
                ...
                <Select defaultValue={field.value} onValueChange={field.onChange}>
                  ...
                  <FormControl>
                    {type === 'CREATE' ? (
                      ...
                      <div className={styles.selectItem}>{getTeamName(field.value)}</div>
                    )}
                  </FormControl>
                  <SelectContent>
