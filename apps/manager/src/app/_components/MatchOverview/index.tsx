        ...
        <Fragment key={league.id}>
          <div className={styles.leagueContainer}>
            <span className={styles.leagueContent}>
              <Tag colorScheme={league.state === '진행 중' ? 'red' : 'secondary'}>
                {league.state}
              </Tag>
              ...
              <Card.Root>
                ...
                <Card.Footer>
                  <Button colorScheme="secondary" size="xs" asChild fullWidth>
                    <Link href={`/league/${league.id}/${game.id}/timeline`}>타임라인 수정</Link>
                  </Button>
                  <Button colorScheme="secondary" size="xs" asChild fullWidth>
                    <Link href={`/league/${league.id}/${game.id}`}>경기 관리</Link>
                  </Button>
                </Card.Footer>
              </Card.Root>
              {league.inProgressGames.length - 1 !== index && <Divider height={1} />}
            </Fragment>
          ))}
