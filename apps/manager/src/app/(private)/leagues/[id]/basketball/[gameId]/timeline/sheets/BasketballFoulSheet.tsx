...
export default function BasketballFoulSheet({
  gameId,
  onClose,
}: {
  leagueId: number;
  ...
  const submit = () => {
    ...
    createWarning(request, {
      onSuccess: () => {
        toast.success('파울이 등록되었습니다.');
        onClose();
      },
      onError: () => {
        toast.error('파울 등록에 실패했습니다. 다시 시도해주세요.');
      },
    });
  };
  ...
  return (
    <div className="flex h-full flex-col gap-4 bg-white p-5">
      ...
      <div className="text-base font-medium text-black">파울 상세 정보</div>
