...
export const PlayerDeleteDialog = ({ id, children, onSuccess, onError, onSettle }: Props) => {
  const { mutateAsync } = useDeletePlayers();

  const handlePlayerDelete = async (): Promise<void> => {
    try {
  ...
  return (
    <AlertDialog
      ...
      primaryTitle="삭제"
      onPrimaryClick={handlePlayerDelete}
    >
      {children}
    </AlertDialog>
  );
